...
async def calc_by_stars_process(message: types.Message, state: FSMContext, repo: Repository, config: Config):
    """
    ...
    # Получаем актуальную цену одной звезды (статическую или динамическую).
...
async def calc_by_rub_process(message: types.Message, state: FSMContext, repo: Repository, config: Config):
    """
    ...
    # Достаем актуальную стоимость звезды (статическую или динамическую).
    star_price = await star_pricing_service.get_star_price(repo, config)
    if star_price == 0:
